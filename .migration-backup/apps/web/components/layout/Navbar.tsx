    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md shadow-sm">
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 pb-6 pt-4 space-y-2">
          <Link href="/listings" className="block py-2 text-gray-700 font-medium">Properties</Link>
          <Link href="/how-it-works" className="block py-2 text-gray-700 font-medium">How It Works</Link>
          {user ? (
            <Link href="/dashboard" className="btn-primary block text-center mt-2">Dashboard</Link>
          ) : (
            <>
              <Link href="/auth/login" className="btn-outline block text-center">Sign In</Link>
              <Link href="/auth/register" className="btn-primary block text-center mt-2">Get Started</Link>
            </>
          )}
        </div>
      )}
