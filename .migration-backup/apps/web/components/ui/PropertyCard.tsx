...
export function PropertyCard({ listing, className, index = 0, isSaved, onToggleSave }: Props) {
    ...
    <div className={cn("property-card group", className)}>
      <Link href={`/listings/${listing.slug || listing.id}`}>
        <div className="relative h-56 overflow-hidden bg-gray-100">
          {img ? (
            <Image src={img.thumbnailUrl || img.url} alt={listing.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, 33vw" />
          ) : (
            <div className="absolute inset-0 flex items-center justify-center bg-kunda-50"><span className="text-4xl">🏠</span></div>
          )}
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            <span className="badge bg-white/90 text-gray-700 text-xs">{listing.propertyType?.charAt(0) + listing.propertyType?.slice(1).toLowerCase()}</span>
            {listing.titleDeedAvailable && <span className="badge bg-kunda-700 text-white text-xs"><Shield className="w-3 h-3" /> Title Deed</span>}
          </div>
          {onToggleSave && (
            <button onClick={(e) => { e.preventDefault(); onToggleSave(listing.id); }} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow-sm">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center gap-1 text-gray-500 text-sm mb-3">
            <MapPin className="w-3.5 h-3.5" /><span className="truncate">{listing.area ? `${listing.area}, ` : ""}{listing.region}</span>
          </div>
