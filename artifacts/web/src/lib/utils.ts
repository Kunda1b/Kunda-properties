...
export function getInitials(f: string, l: string) {
  return `${f[0] || ""}${l[0] || ""}`.toUpperCase();
}

export const PROPERTY_TYPE_LABELS: Record<string, string> = {
  HOUSE: "House", APARTMENT: "Apartment", LAND: "Land",
  COMMERCIAL: "Commercial", VILLA: "Villa", COMPOUND: "Compound",
};
...
export const ESCROW_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  FUNDED: { label: "Funded", color: "bg-yellow-100 text-yellow-700" },
  INSPECTING: { label: "Inspection Period", color: "bg-orange-100 text-orange-700" },
  APPROVED: { label: "Approved", color: "bg-green-100 text-green-700" },
  DISPUTED: { label: "Disputed", color: "bg-red-100 text-red-700" },
  RELEASED: { label: "Complete", color: "bg-kunda-100 text-kunda-700" },
