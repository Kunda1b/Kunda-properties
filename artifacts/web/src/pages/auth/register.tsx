          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-3 gap-2">
              {(["BUYER", "SELLER", "AGENT"] as const).map((r) => (
                <label key={r} className={cn("flex items-center justify-center p-3 rounded-lg border-2 cursor-pointer text-sm font-medium",
                  role === r ? "border-kunda-700 bg-kunda-50 text-kunda-700" : "border-gray-200 text-gray-600")}>
            ...
            {errors.password && <p className="text-red-500 text-xs -mt-2">Password must be 8+ chars with uppercase, lowercase and number</p>}
